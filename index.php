...
<body class="bg-dark">
  <nav class="navbar bg-body-tertiary mb-5">
    <div class="container-fluid">
      ...
      <form class="d-flex" role="search" action="buscarIdeias.php" method="post">
        <input class="form-control me-2" type="search" name="termo_busca" placeholder="Busque aqui..." aria-label="Search">
        <button class="btn btn-outline-success" name="buscar" type="submit">Procurar</button>
      </form>
    </div>
  </nav>
  <div class="w-100 d-flex justify-content-start align-items-center mb-3">
    <a href="perfil.php" class="btn btn-primary m-5">Ir para o perfil</a>
  </div>
  <div class="feed mb-5">
    <?php
    require_once('conexao.php');
    ...
    if (mysqli_num_rows($resultado) > 0) {
      while ($row = mysqli_fetch_assoc($resultado)) {
        ...
        echo '    <div class="d-flex justify-content-center align-items-center modal-content bg-secondary shadow border border-light-subtle">';
  ...
  <script>
    ...
    modalButtons.forEach((button, index) => {
      button.addEventListener('click', () => {
        modals[index].style.display = 'none'; // Fechar o modal
        notification.style.display = 'block'; // Mostrar a notificação
        setTimeout(() => {
          notification.style.display = 'none'; // Esconder a notificação depois de 3 segundos
        }, 3000);
      });
    });
  </script>
