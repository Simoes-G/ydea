...
$sqlMinhasIdeias = "SELECT * FROM perfil WHERE usuario_id = '$iduser'";
$resultadoMinhasIdeias = $conexao->query($sqlMinhasIdeias);

$minhasIdeias = array();
if ($resultadoMinhasIdeias->num_rows > 0) {
  while ($row = $resultadoMinhasIdeias->fetch_assoc()) {
    $ideiaId = $row['idPerfil'];
  ...
  <div class="container">
    ...
    <div class="favorite-ideas d-flex justify-content-center align-items-center gap-5 flex-wrap">
      ...
      if (!empty($ideiasFavoritadas)) {
        foreach ($ideiasFavoritadas as $index => $ideia) {
          ...
          echo '<form action="editarIdeias.php" method="post">';
          ...
          echo '</button>';
          echo '</div>';
          echo '<div class="d-flex flex-column gap-3 modal-body">';
          echo '<img class="w-50" src="imagens/' . $ideia['img'] . '" alt="' . $ideia['titulo'] . '">';
          echo '<h3>Titulo</h3>';
          echo '<input type="text" name="titulo['. $index .']" class="modal-title" id="modal-' . $ideia['idPerfil'] . '-label" value="'. $ideia['titulo'] .'">';
          echo '<h3>Descricao</h3>';
          echo '<input type="text" name="descricao['. $index .']" value="'. $ideia['descricao'] .'">';
          echo '<input type="hidden" name="idIdeia['. $index .']" value="'. $ideia['idPerfil'] .'">';
          echo '<input class="btn btn-success" type="submit" id="salvar" name="salvar" value="Salvar">';
          echo '<input class="btn btn-danger" type="submit" id="deletar" name="deletar" value="Deletar">';
          echo '</div>';
          echo '</div>';
          echo '</div>';
          echo '</form>';
